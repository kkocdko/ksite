        ...
        while let Some(token) = s.peek() {
            match (&token.0, &token.1[..]) {
                ...
                (TokenKind::Word, _) if field.data_type.is_empty() => {
                    ...
                    if !is_normal_type(&field.data_type) && !field.repeated {
                        field.optional = true;
                    }
                }
...
fn is_normal_type(i: &Vec<u8>) -> bool {
    match &i[..] {
        b"double" => true,
        b"float" => true,
        b"int32" => true,
...
fn translate(i: Vec<u8>) -> Vec<u8> {
    let mut src = i.into_iter().peekable();
    ...
    for message in package.messages {
        ...
        for field in message.fields {
            // attr macro
            out.extend(b"    #[prost(");
            if is_normal_type(&field.data_type) {
